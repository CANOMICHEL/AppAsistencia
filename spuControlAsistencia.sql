...
begin
	-- Validacion del año curricular
	if (@Fecha != '' and not exists (
	select * 
		from 	TAsistenciaAlumno 
		where day(Fecha) = day(@Fecha) and 
...
--=======================================================================================
-- SPU ultimocod
if exists (select * from dbo.sysobjects where name = 'spuTAlumno_Ultimo')
	drop procedure spuTAlumno_Ultimo
go
create procedure spuTAlumno_Ultimo
as
begin
	select max(CodAlumno) from TAlumno
end
go

if exists (select * from dbo.sysobjects where name = 'spuTDocente_Ultimo')
	drop procedure spuTDocente_Ultimo
go
create procedure spuTDocente_Ultimo
as
begin
	select max(CodDocente) from TDocente
end
go
